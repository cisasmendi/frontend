...
<Accordion title={"Inscripciones"}>
    <div slot="content">
        {#if programInfo?.contact?.email || programInfo?.contact?.wp || programInfo?.contact?.phone}
            <h4 class="text-lg font-semibold text-[#6d0205]">Contacto</h4>
            <ul>
                {#if programInfo?.contact?.email}

                    <li class="flex items-center">
                        Escribinos a: &nbsp
                        <a
                            href="mailto:{programInfo.contact.email}"
                            class="a-oscuro"
                            target="_blank"> {programInfo.contact.email}</a
                        >
                    </li>
                {/if}
                {#if programInfo?.contact?.phone}
                <li class="flex items-center">
                    Comunicate por telefono al &nbsp
                        <a
                            href="tel:{programInfo.contact.phone}"
                            class="a-oscuro"
                            target="_blank">{programInfo.contact.phone}</a
                        >
                    </li>
                {/if}
                {#if programInfo?.contact?.wp}
                    <li class="flex items-center">
                        Comunicate por WhatsApp al
                        <a
                            href="https://wa.me/{programInfo.contact.wp}"
                            target="_blank"
                            class="flex items-center ml-2"
                        >
                            <span>{programInfo.contact.wp}</span>
                            <svg
                                class="ml-2 cursor-pointer rounded-md shadow-md transition-all duration-300 hover:shadow-green-200"
        ...
        {#if programInfo?.registrations?.length > 0}
        <h4 class="text-lg font-semibold text-[#6d0205]">Links:</h4>
            {#each programInfo.registrations as registration}
                <li>
                    <a href={registration.url} class="text-blue-600 s-HL7iPF8fmwQi" target="_blank"
                        ><i class="fa fa-link"></i> {registration.name}</a
                    >
                </li>
                <br />
            {/each}
